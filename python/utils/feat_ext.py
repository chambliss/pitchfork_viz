...
def get_unique_artists(artists_df: pd.DataFrame) -> pd.DataFrame:
    ...
    unique_artists = artists_df.groupby('artist') \
                               .agg(lambda x: str(x.values)) \
                               .reset_index()

    # Turn the string of reviewids into a list of ids
    unique_artists['reviewid'] = unique_artists['reviewid'] \
                                                .str.strip('[]').str.split()
...
def get_unique_genres(genres_df: pd.DataFrame) -> pd.DataFrame:
    ...
    unique_genres = genres_df['genre'].drop_duplicates() \
                                      .to_frame('genre') 
...
def get_unique_authors(reviews_df: pd.DataFrame) -> pd.DataFrame:
    ...
    unique_authors = reviews_df[['author', 'author_type']] \
                                    .drop_duplicates() \
...
def create_ents_df(spacy_doc: Doc, review_id: int) -> pd.DataFrame:
    ...
    ents_in_text = [(review_id, ent.text, ent.label_) for ent in spacy_doc.ents
...
def create_pos_df(spacy_doc: Doc, review_id: int) -> pd.DataFrame:
    ...
    pos_tags_in_text = [(review_id, tok.text, tok.pos_) for tok in spacy_doc 
...
def extract_text_features(content_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    
    start_time = time.time()
    id_text_pairs = content_df[['reviewid', 'content']].values
    nlp = spacy.load('en_core_web_lg')
    
    combined_ents_df, combined_pos_df = pd.DataFrame(), pd.DataFrame()
    
    for i, (review_id, text) in enumerate(id_text_pairs, start=1):
        doc = nlp(text)
        ...
        if i % 1000:
            ...
            print(f'Finished {i} reviews. Time elapsed: {time.time() - start_time}')
            
            # Remove the last set of temp csv files
            if os.path.exists(f"../data/ents_{i}.csv"):
